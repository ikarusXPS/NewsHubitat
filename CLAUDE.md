...
| `analysis.spec.ts` × 1 (should display cluster summaries section) | beforeEach 15s budget for `h1:has-text("PERSPEKTIVEN-ANALYSE")` blows when this test slot lands while `/analysis` is still resolving RequireAuth + useClusters fetch under parallel CI load. Test body is a no-op anyway (`if (await clusterSection.isVisible())` soft check); cluster-summary surface is covered by ClusterSummary unit tests. Re-enable once beforeEach pre-resolves auth + cluster fetch before the h1 paint check |
