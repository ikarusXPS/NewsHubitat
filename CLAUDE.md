...
| ~~`analysis.spec.ts` × 3 (compare modal open/close + cluster summaries)~~ | **Re-enabled 2026-05-11 (todo 40-13).** `data-testid="analysis-ready"` hydration anchor added to `AnalysisPage` root; `beforeEach` waits on the anchor (20s budget) instead of the framer-motion-animated h1. Same anchor also closes the `navigation.spec.ts:32` flake. |
