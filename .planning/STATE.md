...
- Badge seed data uses uniqueName pattern (name-tier) for database uniqueness
- Achievement detection via ref-based previous stats comparison
- 5-second debounce on achievement checking
- Badge groups show highest earned or first unearned tier (collapsed view)
- Featured badge per D-43 allows user to select one badge to display prominently
